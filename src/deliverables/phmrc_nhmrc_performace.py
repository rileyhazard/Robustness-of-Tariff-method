...
from src.validation import out_of_sample_accuracy, config_sss_model_selector
from src.tariff import TariffClassifier
from src.utils import union_NDFrame_indicies


SRC_DATA_DIR = os.path.join(getters.REPO_DIR, 'src', 'data')
...
def get_phmrc(module):
    data_dir = os.path.join(getters.REPO_DIR, 'data', 'smartva_output',
                            'phmrc_short-3')
...
def get_combined(module):
    data_dir = os.path.join(getters.REPO_DIR, 'data', 'smartva_output',
                            'gs_full')
    gs_path = os.path.join(getters.REPO_DIR, 'external', 'data_release',
                           'gs_full.csv')
...
def main():
    outdir = os.path.join(getters.REPO_DIR, 'data', 'combined_performance')

...
/* TODO:
This does not produce clean 75/25 percent splits. If a test observationi is
sampled zero times while resampling to a uninformative dirichlet distribution
it should remain in the test split. Zeros in splits files are either training
observations or unsampled tested observations. By using both, this algorithm
produces a training dataset between 75% and 89% of the total observations.
Since a larger portion of the data is used for training this leads to
over-estimating performance.
*/ 
preserve
...
/* TODO:
Boostraps were performed using the enitre dataset instead of only the
observations in the training data. Insignificant tariffs are removed because
this reduces noise and improves accuracy. Since significance testing used
information from the test data, this likely produces an over-estimate of
performance.
*/
